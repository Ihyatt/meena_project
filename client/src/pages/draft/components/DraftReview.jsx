...
const DraftReview = () => {
  ...
  const { fetchDraft, saveDraft, shareCampaignDraft, upload, isLoading } =
    useDraftStore();
  ...
  const handleShare = (event) => {
    ...
    shareDraft(campaignId, title, description, goal, closeoutDate.$d);
    navigate(-1);
    event.preventDefault();
  };
  ...
  const handleSave = (event) => {
    ...
    saveDraft(campaignId, title, description, goal, closeoutDate.$d).then(
      (data) => {
        setTitle(data.title);
        setDescription(data.description);
        setGoal(data.goal);
